...
device_path = '/dev/input/event0'  # 替换成你的设备路径
...
class BarcodeScanner:
    ...
    def _handle_key_event(self, event):
        ...
        if event.code in KEY_MAPPING:
            self.scanned_data += str(KEY_MAPPING[event.code][0])
        elif event.code == ecodes.KEY_ENTER:
            self.scanned_data = self.scanned_data.upper()
            oled.monitor(label=self.scanned_data, voltage=Voltage)
            self.label = self.scanned_data
            self.scanned_data = ''
